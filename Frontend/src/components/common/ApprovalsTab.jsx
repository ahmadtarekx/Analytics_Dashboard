/**
 * ApprovalsTab.jsx — Tab Component (Strategy Pattern)
 *
 * Extracted from ManagerDashboardStrategy.jsx / old Dashboard.jsx.
 *
 * Renders the full Pending Approvals view, adapting to the caller's role:
 *
 *   Owner         → 3 sub-tabs: Manager Approvals | PR Campaigns | Expense Approvals
 *   Finance Mgr   → 2 sub-tabs: Manager Approvals | Pending Reports
 *   Other Managers→ 1 view:     Manager Approvals only
 *
 ...
 *   approvals              {Array}
 *   apprLoading            {boolean}
 *   onResolveApproval      {Function(ticketId, action)}
 *
 *   // PR Campaigns (owner only)
 *   ownerCampaigns         {Array}
 *   ownerCampaignsLoading  {boolean}
 *   onResolveCampaign      {Function(ticketId, action)}
 *
 *   // Expense approvals (owner / finance manager)
 *   approvalExpenses        {Array}
 ...
 *
 *   // Refresh
 *   onRefresh              {Function}   — reloads all data for the current role
 */
...
export default function ApprovalsTab({
  ...
  const refreshing = apprLoading || ownerCampaignsLoading || approvalExpensesLoading;

  return (
    <div style={{ animation: 'fadeUp 0.3s ease' }}>
      <SectionHead title={language === 'ar' ? 'الموافقات المعلقة' : 'Pending Approvals'} />
      ...
      {isOwner && (
        <div style={{
          ...
          {[
            ...
            <button
              key={t.id}
              onClick={() => setApprovalSubTab?.(t.id)}
              style={{
                display: 'flex', alignItems: 'center', gap: 7,
                padding: '9px 18px', borderRadius: 10, cursor: 'pointer',
                fontFamily: 'inherit', fontWeight: 700, fontSize: 13, transition: 'all .2s',
                background: approvalSubTab === t.id ? 'rgba(79,124,255,0.12)' : 'transparent',
                border: `1.5px solid ${approvalSubTab === t.id ? 'rgba(79,124,255,0.4)' : 'transparent'}`,
                color: approvalSubTab === t.id ? 'var(--accent2)' : 'var(--txt3)',
              }}
            >
              <Icon d={t.icon} size={14} color={approvalSubTab === t.id ? 'var(--accent2)' : 'var(--txt3)'} />
              {t.label}
              {t.id === 'pr_campaigns' && ownerCampaigns.length > 0 && (
                <span style={{
                  fontSize: 10, fontWeight: 800,
                  background: 'rgba(239,68,68,0.2)', color: '#fca5a5',
                  border: '1px solid rgba(239,68,68,0.3)', padding: '1px 7px', borderRadius: 999, marginLeft: 4,
                }}>
                  {ownerCampaigns.length}
                </span>
              )}
              {t.id === 'expense_approvals' && approvalExpenses.length > 0 && (
                <span style={{
                  fontSize: 10, fontWeight: 800,
                  background: 'rgba(239,68,68,0.2)', color: '#fca5a5',
                  border: '1px solid rgba(239,68,68,0.3)', padding: '1px 7px', borderRadius: 999, marginLeft: 4,
          ...
          >
            {refreshing ? <span className="spinner" style={{ width: 11, height: 11 }} /> : '↻'} Refresh
          </button>
        </div>
      )}

      {/* ── Finance Manager sub-tabs ── */}
              ...
              >
                ...
                {t.id === 'approvals' && ownerApprovedCount > 0 && (
                  <span style={{
                    ...
                    border: '1px solid rgba(16,185,129,0.3)', padding: '1px 7px', borderRadius: 999, marginLeft: 4,
                  }}>
                    {ownerApprovedCount}
                  </span>
                )}
                {t.id === 'expense_approvals' && pendingCount > 0 && (
                  <span style={{
                    fontSize: 10, fontWeight: 800,
                    background: 'rgba(239,68,68,0.2)', color: '#fca5a5',
                    border: '1px solid rgba(239,68,68,0.3)', padding: '1px 7px', borderRadius: 999, marginLeft: 4,
      ...
      {/* ══ PR CAMPAIGNS SUB-TAB (Owner only) ══ */}
      {isOwner && approvalSubTab === 'pr_campaigns' && (
        <div>
          {ownerCampaignsLoading && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--txt3)', fontSize: 13 }}>
              <span className="spinner" style={{ width: 15, height: 15 }} /> Loading campaigns...
            </div>
          )}
          {!ownerCampaignsLoading && ownerCampaigns.length === 0 && (
            <div style={{
              padding: '52px', textAlign: 'center', color: 'var(--txt3)',
          ...
          {ownerCampaigns.map(c => {
            ...
            const typeLabel  = isPR ? 'Press Release' : (c.campaign_type || 'Campaign');
            const titleText  = isPR ? c.title : c.campaign_name;
              ...
              <div key={c.ticket_id} className="ticket-card" style={{ marginBottom: 14 }}>
                ...
                <div style={{ padding: '18px 22px' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, flexWrap: 'wrap' }}>
                        <span style={{
                          ...
                          background: `${col}18`, color: col, border: `1px solid ${col}35`, whiteSpace: 'nowrap',
                        }}>
                          {typeLabel}
                        </span>
                        <span style={{ fontSize: 15, fontWeight: 800, color: 'var(--txt)' }}>{titleText || '—'}</span>
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--txt3)' }}>
                        By <strong style={{ color: 'var(--txt2)' }}>{c.emp_name}</strong>
                        {' · '}{fmtDate(c.submitted_at || c.time)}
                      </div>
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--txt3)', marginLeft: 12, whiteSpace: 'nowrap' }}>
                      #{c.ticket_id}
                    </div>
                  </div>
                      ...
                      <div style={{
                        ...
                        {c.target_date && (
                          <div style={{ fontSize: 11, color: 'var(--txt3)', padding: '6px 10px', background: 'var(--surface2)', borderRadius: 8 }}>
                            <strong style={{ color: 'var(--txt2)' }}>📅 Release Date:</strong> {c.target_date}
                          </div>
                        )}
                        {c.media_outlets && (
                          <div style={{ fontSize: 11, color: 'var(--txt3)', padding: '6px 10px', background: 'var(--surface2)', borderRadius: 8 }}>
                            <strong style={{ color: 'var(--txt2)' }}>📡 Media Outlets:</strong> {c.media_outlets}
                          </div>
                        )}
                      </div>
                      {c.content && (
                        <div style={{
                          fontSize: 13, color: 'var(--txt2)', padding: '12px 14px',
                          background: 'var(--surface2)', borderRadius: 10, marginBottom: 14,
                  ...
                  {!isPR && (
                    <div>
                      <div style={{
                        ...
                        {c.platform && (
                          <div style={{ fontSize: 11, color: 'var(--txt3)', padding: '6px 10px', background: 'var(--surface2)', borderRadius: 8 }}>
                            <strong style={{ color: 'var(--txt2)' }}>🌐 Platform:</strong> {c.platform}
                          </div>
                        )}
                        {c.budget > 0 && (
                          <div style={{ fontSize: 11, color: 'var(--txt3)', padding: '6px 10px', background: 'var(--surface2)', borderRadius: 8 }}>
                            <strong style={{ color: 'var(--txt2)' }}>💰 Budget:</strong> {Number(c.budget).toLocaleString()} EGP
                          </div>
                        )}
                        {(c.campaign_start_date || c.campaign_end_date) && (
                          <div style={{ fontSize: 11, color: 'var(--txt3)', padding: '6px 10px', background: 'var(--surface2)', borderRadius: 8 }}>
                            <strong style={{ color: 'var(--txt2)' }}>📅 Duration:</strong> {c.campaign_start_date || '?'} → {c.campaign_end_date || '?'}
                          </div>
                        )}
                        {c.target_audience && (
                          <div style={{ fontSize: 11, color: 'var(--txt3)', padding: '6px 10px', background: 'var(--surface2)', borderRadius: 8 }}>
                            <strong style={{ color: 'var(--txt2)' }}>🎯 Audience:</strong> {c.target_audience}
                          </div>
                        )}
                      </div>
                      {c.notes && (
                        <div style={{
                          fontSize: 12, color: 'var(--txt2)', padding: '10px 12px',
                          background: 'var(--surface2)', borderRadius: 8, marginBottom: 12,
                          lineHeight: 1.6, borderLeft: `3px solid ${col}`,
                        }}>
                  ...
                  <div style={{ display: 'flex', gap: 10 }}>
                    <button
                      onClick={() => onResolveCampaign?.(c.ticket_id, 'approve')}
                      style={{
                    ...
                    >
                      ...
                      {isPR ? 'Approve Release' : 'Approve Campaign'}
                    </button>
                    <button
                      onClick={() => onResolveCampaign?.(c.ticket_id, 'reject')}
                      style={{
                        flex: 1, padding: '10px', borderRadius: 9,
      ...
      {/* ══ EXPENSE APPROVALS SUB-TAB (Owner: pending_owner | Finance Manager: pending_manager) ══ */}
      {((isOwner && approvalSubTab === 'expense_approvals') ||
        (isFinanceManager && !isOwner && approvalSubTab === 'expense_approvals')) && (
        <div>
          {approvalExpensesLoading && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--txt3)', fontSize: 13 }}>
              <span className="spinner" style={{ width: 15, height: 15 }} /> Loading expense reports...
            </div>
          )}
          {!approvalExpensesLoading && approvalExpenses.filter(e => e.stage !== 'approved_by_owner').length === 0 && (
            <div style={{
              padding: '52px', textAlign: 'center', color: 'var(--txt3)',
              background: 'var(--surface)', border: '1px dashed var(--border)', borderRadius: 16,
            }}>
          ...
          {approvalExpenses
            .filter(exp => exp.stage !== 'approved_by_owner')
            .map(exp => {
              ...
              return (
                <div key={exp.ticket_id} className="ticket-card" style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ padding: '18px 22px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--txt3)' }}>
                          ...
                          {' · '}{fmtDate(exp.submitted_at || exp.time)}
                        </div>
                      </div>
                    ...
                    {exp.description && (
                      <div style={{
                        fontSize: 13, color: 'var(--txt2)', padding: '10px 12px',
                        background: 'var(--surface2)', borderRadius: 8, marginBottom: 12,
                        lineHeight: 1.6, borderLeft: `3px solid ${col}`,
                      }}>
                        {exp.description}
      ...
      {/* ══ MANAGER APPROVALS (standard — shown when sub-tab is 'approvals' or user is a plain manager) ══ */}
        ...
        <div>
          {apprLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {approvals.map((appr, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ animationDelay: `${i * 0.05}s`, display: 'flex', alignItems: 'stretch' }}
                  >
                    <div style={{
                    ...
                    <div style={{ padding: '22px 26px', flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                        {submitter && (
                          <div style={{
                            flex: 1, minWidth: 160, padding: '11px 14px', borderRadius: 10,
                            ...
                            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--txt)' }}>{submitter.name}</div>
                            {submitter.id && <div style={{ fontSize: 11, color: 'var(--txt3)', marginTop: 2 }}>ID #{submitter.id}</div>}
                          </div>
                        )}
                        {data.target_emp_id && (
                          <div style={{
                            ...
                            {isSalary && data.adjustment && (
                              <div style={{ fontSize: 11, color: 'var(--txt3)', marginTop: 2 }}>
                                {data.adjustment.type === 'bonus' ? '🟢 Bonus' : '🔴 Deduction'}
                                {' · '}{data.adjustment.amount?.toLocaleString()} EGP
                              </div>
                            )}
                          </div>
                        )}
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                            {Object.entries(changes).map(([field, val]) => (
                              <div
                                key={field}
                                style={{
                                  display: 'grid', gridTemplateColumns: '130px 1fr 20px 1fr',
                              ...
                              >
                                ...
                                <div style={{
                                  ...
                                  fontSize: 12, color: '#f87171', fontWeight: 600, wordBreak: 'break-all',
                                }}>
                                  {val.before}
                                </div>
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--txt3)" strokeWidth="2.5" strokeLinecap="round">
                                  <path d="M5 12h14M13 6l6 6-6 6" />
                                </svg>
                                <div style={{
                                  padding: '5px 10px', borderRadius: 7,
                                  background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.22)',
                                  fontSize: 12, color: '#34d399', fontWeight: 600, wordBreak: 'break-all',
                                }}>
                                  {val.after}
                                </div>
                              </div>
                      ...
                      {data.action === 'DELETE_PRODUCT' && data.payload && (
                        <div style={{
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: 8 }}>
                            {[
                              ...
                              <div key={fi} style={{ padding: '8px 12px', borderRadius: 9, background: 'var(--surface)', border: '1px solid var(--border)' }}>
                          ...
                          {data.payload.amount_avail > 0 && (
                            <div style={{
                              ...
                              <strong>Warning:</strong> This product still has {Number(data.payload.amount_avail).toLocaleString()} units in stock.
                            </div>
                          )}
                        ...
                        <div style={{
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
                            {[
                              ...
                              <div key={f.label} style={{ padding: '10px 12px', borderRadius: 8, background: 'var(--surface2)', border: '1px solid var(--border)' }}>
                                ...
                                <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--txt)' }}>{f.value}</div>
                      ...
                      <div style={{ display: 'flex', gap: 10 }}>
                        <button
                          onClick={() => onResolveApproval?.(appr.ticket_id, 'approve')}
                          style={{
                            padding: '11px 20px', borderRadius: 9,
                            background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.3)',
                            color: '#10b981', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit',
                            flex: 1, fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
                          }}
                        >
                          <Icon d={IC.check} size={13} color="#10b981" />
                          {isDeleteTicket
                            ? 'Approve Deletion'
                            : isSalary
                              ? `Approve ${data.adjustment?.type === 'bonus' ? 'Bonus' : 'Deduction'}`
                              : isLeave
                                ? 'Approve & Apply Leave'
                                : 'Approve & Apply'}
                        </button>
                        <button
                          onClick={() => onResolveApproval?.(appr.ticket_id, 'reject')}
                          style={{
                            padding: '11px 20px', borderRadius: 9,
                            background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)',
                            color: '#ef4444', fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit',
              ...
              {/* Finance Manager: Owner-approved expense notifications */}
                      ...
                      <div key={exp.ticket_id || i} className="ticket-card" style={{ marginBottom: 10, opacity: 0.85 }}>
                        ...
                        <div style={{ padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                          ...
                          <span style={{ fontSize: 11, color: 'var(--txt3)' }}>
                            by <strong style={{ color: 'var(--txt2)' }}>{exp.emp_name}</strong>
                            {' · '}{fmtDate(exp.approved_at || exp.submitted_at)}
                          </span>
                          {exp.description && (
                            <span style={{ fontSize: 12, color: 'var(--txt3)', marginLeft: 'auto' }}>{exp.description}</span>
                          )}
                        </div>
                      </div>
                    );
